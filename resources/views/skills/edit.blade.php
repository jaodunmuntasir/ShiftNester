    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden p-6">
        <form action="{{ route('skills.update', $skill) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ $skill->name }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>
            <div class="mb-4">
                ...
                <select name="is_boolean" id="is_boolean" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    <option value="0" {{ $skill->is_boolean ? '' : 'selected' }}>Rated (1-5)</option>
                    <option value="1" {{ $skill->is_boolean ? 'selected' : '' }}>Boolean (Yes/No)</option>
                </select>
            </div>
