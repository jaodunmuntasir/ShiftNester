        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden mt-6 p-6">
            <form action="{{ route('skills.update', $skill) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" value="{{ $skill->name }}" class="shadow-sm focus:ring-red-500 focus:border-red-500 block w-full sm:text-sm border-gray-300 rounded-md" required>
                </div>
                <div class="mb-6">
                    <span class="block text-gray-700 text-sm font-bold mb-2">Skill Type:</span>
                    <div class="mt-2 space-y-2">
                        <div class="flex items-center">
                            <input id="rated" name="is_boolean" type="radio" value="0" class="focus:ring-red-500 h-4 w-4 text-red-600 border-gray-300" {{ $skill->is_boolean ? '' : 'checked' }} required>
                            <label for="rated" class="ml-3 block text-sm font-medium text-gray-700">Rated (1-5)</label>
                        </div>
                        <div class="flex items-center">
                            <input id="boolean" name="is_boolean" type="radio" value="1" class="focus:ring-red-500 h-4 w-4 text-red-600 border-gray-300" {{ $skill->is_boolean ? 'checked' : '' }} required>
