    ...
    <div class="py-8">
        <div class="flex flex-col md:flex-row justify-between items-center mb-6">
            ...
            <div class="flex flex-col sm:flex-row w-full md:w-auto">
                <form method="GET" action="{{ route('departments.index') }}" class="flex-grow mb-4 sm:mb-0 sm:mr-4">
                    <div class="flex">
                        <input type="text" name="search" placeholder="Search departments" value="{{ request('search') }}" class="w-full px-4 py-2 rounded-l-lg border-t border-b border-l text-gray-800 border-gray-200 bg-white focus:outline-none focus:border-red-500" />
                ...
                <a href="{{ route('departments.create') }}" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110 text-center">
                    Add New Department
                </a>
            </div>
        </div>
                ...
                <thead>
                    <tr>
                        <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                            <a href="{{ route('departments.index', ['sort' => 'name', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}" class="flex items-center">
                                Name
